...
void Arm::UpdateCurrentHandOffset() {
    if (IsAttached()) {
        ...
        if (m_pHeldMO && !dynamic_cast<ThrownDevice *>(m_pHeldMO)) {
			...
			float minrecoil = std::clamp(heldDevice->GetVisualRecoilMultiplier(), 0.0F, 0.8F);

			float recoilScalar = std::min((heldDevice->GetRecoilForce() * heldDevice->GetVisualRecoilMultiplier() / totalGripStrength).GetMagnitude() * 0.4F, minrecoil) * rotAngleScalar;
